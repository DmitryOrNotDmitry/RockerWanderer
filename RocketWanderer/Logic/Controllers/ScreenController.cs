  ...
  public abstract class ScreenController : BaseController
  {
    ...
    public ScreenController(WindowData parWindowData)
    {
      ...
      _descriptionScreen = new DescriptionScreen(
        """
        В данной игре вам предстоит управлять космическим кораблем, который может перемещаться между орбитами планет. Управление кораблем происходит с помощью кнопки «Пробел», при нажатии на который корабль перестает двигаться по орбите планеты и начинает двигаться по прямой. Ваша задача – попасть кораблем на орбиту другой планеты. Игра заканчивается, когда корабль врезается в планету, солнце или пояс астероидов.
        """
        );
